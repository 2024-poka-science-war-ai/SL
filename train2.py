...
def train_loop(dataloader, model, optimizer, epoch, epochs, scheduler=None):
    ...
    for batch, data in enumerate(dataloader):
        ...
        button_preds, main_stick_pred, c_stick_pred, trigger_pred, _ = actor(torch.permute(torch.concat([X[:,:SEQ_LEN*2,:].to(device), y[:,D:D+SEQ_LEN*2,:].to(device)], axis=2).float(), (1, 0, 2)), init_state)
        ...
        for idx, button_pred in enumerate(button_preds):
            button_pred = button_pred.permute(1, 0, 2)[:, D+SEQ_LEN:, 0]  ## D = delay, seq_len = SEQ_LEN
            # 입력과 타겟의 차원을 맞추기 위해 permute 사용
            ## D+1:D+1+SEQ_LEN -> D+1+SEQ_LEN 이렇게 하면 마지막 프레임으로만 loss를 구함 -> 이상한 hidden state의 영향 감소, button_pred도 똑같이 [:, -1]
            ce_btn_loss = bce(button_pred, y[:, D+1+SEQ_LEN:D+1+SEQ_LEN*2, idx].to(device)) ## y = Batch_size X seq_len X action종류
            btn_loss += ce_btn_loss.mean()
        ...
        # Main stick loss 계산
        main_stick_loss = ce(main_stick_pred.permute(1, 0, 2)[:, D+SEQ_LEN:, :].reshape(-1, main_stick_pred.size(2)), 
                                        y[:, D+1+SEQ_LEN:D+1+SEQ_LEN*2, 5].reshape(-1).type(torch.int64).to(device))

        # C-stick loss 계산
        c_stick_loss = ce(c_stick_pred.permute(1, 0, 2)[:, D+SEQ_LEN:, :].reshape(-1, c_stick_pred.size(2)), 
                                    y[:, D+1+SEQ_LEN:D+1+SEQ_LEN*2, 6].reshape(-1).type(torch.int64).to(device))

        # Trigger loss 계산
        trigger_loss = ce(trigger_pred.permute(1, 0, 2)[:, D+SEQ_LEN:, :].reshape(-1, trigger_pred.size(2)), 
                                    y[:, D+1+SEQ_LEN:D+1+SEQ_LEN*2, 7].reshape(-1).type(torch.int64).to(device))
        ...
        ## 1+:SEQ_LEN+1은 St+1
            V_, _ = target_net(torch.permute(torch.concat([X[:,1:SEQ_LEN*2+1,:].to(device), y[:,D+1:D+SEQ_LEN*2+1,:].to(device)], axis=2).float(), (1, 0, 2)), init_state)
            target = V_.permute(1, 0, 2).reshape(-1, 1).float() + R[:, D+1:D+1+SEQ_LEN*2].reshape(-1, 1).float().to(device) ## 얘네도 no grad 아래인 게 좋을 듯
        V, _ = critic(torch.permute(torch.concat([X[:,:SEQ_LEN*2,:].to(device), y[:,D:D+SEQ_LEN*2,:].to(device)], axis=2).float(), (1, 0, 2)), init_state)   
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            ...
            button_preds, main_stick_pred, c_stick_pred, trigger_pred, _ = actor(
                torch.permute(torch.concat([X[:,:SEQ_LEN,:].to(device), y[:,D:D+SEQ_LEN,:].to(device)], axis=2), (1, 0, 2)).float(), init_state)
...
if __name__ == "__main__":
    ...
    batch_size = 1024
    config = DatasetConfig(dataset_basepath="/root/multi_purpose/slp_full_data/sample", seq_len=2*SEQ_LEN+D+1)
