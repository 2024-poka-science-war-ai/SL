...
def train_loop(dataloader, model, loss_fn, optimizer, epoch, epochs, scheduler=None):
    ...
    for batch, data in enumerate(dataloader):
        ...
        target = V_.permute(1, 0, 2).reshape(-1, 1).float() + R[:, D+1:D+1+U].reshape(-1, 1).float().to(device)
...
if __name__ == "__main__":
    ...
    train_ratio = 1. #torch.randint(0, 4, (1,)).item()
    throw_away_ratio = 0.
    batch_size = 1024
    ...
    x, _, _ = train_dataset[10]#['p1_state']
    ...
    loss_fn = nn.MSELoss() # 아까 CE라고 생각하고 돌렸던거에 사실 MSE 들어갔었는듯. CE 넣으니까 이상해지는데;;
